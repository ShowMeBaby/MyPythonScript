    ...
    def cell_gradient(self, cell_magnitude, cell_angle):
        ...
        for i in range(cell_magnitude.shape[0]):
            for j in range(cell_magnitude.shape[1]):
                ...
                # 双线性插值
                min_angle, max_angle, weight = self.get_closest_bins(
                    gradient_angle)
                orientation_centers[min_angle] += (
                    gradient_strength * (1 - weight))
        ...
        for x in range(cell_gradient.shape[0]):
            for y in range(cell_gradient.shape[1]):
                ...
                for magnitude in cell_grad:
                    ...
                    x1 = int(x * self.cell_size + cell_width +
                             magnitude * cell_width * math.cos(angle_radian))
                    y1 = int(y * self.cell_size + cell_width +
                             magnitude * cell_width * math.sin(angle_radian))
                    x2 = int(x * self.cell_size + cell_width -
                             magnitude * cell_width * math.cos(angle_radian))
                    y2 = int(y * self.cell_size + cell_width -
                             magnitude * cell_width * math.sin(angle_radian))
